...
specific_date = datetime.strptime("2021-08-15", "%Y-%m-%d")  # Specify the desired date here
...
# Create a new Excel file for the specified date only
for _, date_row in unique_dates.iterrows():
    ...
    if current_date != specific_date:
        continue  # Skip dates that do not match the specified date

    # Calculate the previous ... days
    day_dates = [(current_date - timedelta(weeks=i)) for i in range(train_no + 1)]
    ...
    output_excel_path = rf'C:\Users\tanto\Desktop\Forecast\{folder_name}\{current_date.strftime("%Y-%m-%d")}_Data.xlsx'
